...
use ShopMagicVendor\Getresponse\Sdk\Client\Exception\ConnectException;
use ShopMagicVendor\Getresponse\Sdk\Client\Exception\ExceptionFactory;
use ShopMagicVendor\Getresponse\Sdk\Client\Exception\RequestException;
...
use ShopMagicVendor\Getresponse\Sdk\Client\Version;
use ShopMagicVendor\GuzzleHttp\Psr7\Message;
use ShopMagicVendor\GuzzleHttp\Psr7\Response;
...
class CurlRequestHandler implements RequestHandler
{
    ...
    public function send(Call $call)
    {
        ...
        try {
            ...
            $call->setException(ExceptionFactory::exceptionFrom(ConnectException::CODE, $call->getRequest(), $e->getPrevious()->getMessage(), RequestException::getHandlerInfoFromCurlHandler($curlHandle), Version::VERSION));
    ...
    private function sendCurlRequest(RequestInterface $request, $curlHandle)
    {
        ...
        if (curl_errno($curlHandle) !== 0) {
            throw ExceptionFactory::exceptionFrom(ConnectException::CODE, $request, curl_error($curlHandle), RequestException::getHandlerInfoFromCurlHandler($curlHandle), Version::VERSION);
        }
        return $response;
    }
    /**
    ...
    protected function setUpCurl(RequestInterface $request, $curlHandle)
    {
        ...
        curl_setopt($curlHandle, \CURLOPT_HEADER, \true);
